...
public class Activity2 extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		adapter1 = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, locationNameList1);
		adapter2 = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item, locationNameList2);		
		...
		listaendereco1.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, 	int position, long id) {
				...
				Toast.makeText(getApplicationContext(), "Endereço 1 " + endereco + Latidude1 + Longitude1, Toast.LENGTH_SHORT).show();
				enderecobuscar1.setText(endereco);				
				if(enderecobuscar2.getText()== null){
					Toast.makeText(getApplicationContext(), "Digite o outro endereço", Toast.LENGTH_SHORT).show();
				}
			}
		});
		listaendereco2.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, 	int position, long id) {
				...
				Toast.makeText(getApplicationContext(), "Endereço 2 " + endereco + Latidude2 + Longitude2, Toast.LENGTH_SHORT).show();
				...
				Toast.makeText(getApplicationContext(), "latitude 1 " + Latidude1 + " longitude 1 " + Longitude1, Toast.LENGTH_SHORT).show();
				Toast.makeText(getApplicationContext(), "latitude 2 " + Latidude2 + " longitude 2 " + Longitude2, Toast.LENGTH_SHORT).show();
				DecimalFormat df = new DecimalFormat("#.##");
				double distanciaformatada = Double.parseDouble(df.format(d));
				AlertDialog.Builder altDialog= new AlertDialog.Builder(Activity2.this);
				altDialog.setTitle("Cálculos ");
				altDialog.setMessage("Distância entre os locais " + distanciaformatada); 				
	...
	OnClickListener btendereco2OnClickListener = new OnClickListener(){
		...
		public void onClick(View arg0) {
			...
			if(locationName == null){
				...
				try {
					locationList2 = geocoder2.getFromLocationName(locationName, maxResults);
						...
						if(locationList2.isEmpty()){
							...
							for(Address i : locationList2){
								if(i.getFeatureName() == null){
									...
									locationNameList2.add(i.getFeatureName() + ", "+ i.getLocality()+ ", "+ i.getCountryName());
									//enderecocerto =  String.format("%s, %s, %s",  end.getMaxAddressLineIndex() > 0 ? end.getAddressLine(0) : "", end.getLocality(), end.getCountryName());
	...
	OnClickListener btendereco1OnClickListener = new OnClickListener(){
		...
		public void onClick(View arg0) {
			...
			if(locationName == null){
				...
				try {
					locationList1 = geocoder1.getFromLocationName(locationName, maxResults);
